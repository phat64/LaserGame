...
int Actor::GetAngle(int p[2])
{
	...
	double dist = sqrt((double)( ((p[0] - centerX) * (p[0] - centerX)) + ((p[1] - centerY) * (p[1] - centerY))));
...
int Actor::SortNodesByDistance(Node * nodes, int n)
{
#define BUF_SIZE 1024
	...
	int z = (m_FovAngle - 160) * 4;
	if ( z < 0) z = 0;
	z = 0;
	for (int i = 1 + z; i < BUF_SIZE-1-z ; i ++)
	{
 	 	if (lineIdx[i] >= 0)
 	 	   UsedLines[lineIdx[i]] ++;
...
bool IsErrorNode (Node * n) {
  return Distance(n->from, n->to) < 0.001;
}
...
int Actor::GetFovPoints2(Gfx*g,int *points)
{
 	...
 	int centerX = m_X + CELL_W / 2;
 	int centerY = m_Y + CELL_H / 2;
 	static int r = /*SCREEN_W*/CELL_W*2;
   ...
   // FILE * f = fopen("stdout.txt","w+");
	...
	fovRayStart2[0] = centerX;
	fovRayStart2[1] = centerY;
	fovRayStart2[2] = centerX + (int) (r * cos((fovStartAngle+5) * 2 * M_PI / 360.0));
	fovRayStart2[3] = centerY + (int) (r * sin((fovStartAngle+5) * 2 * M_PI / 360.0));
	
	fovRayEnd2[0] = centerX;
	fovRayEnd2[1] = centerY;
	fovRayEnd2[2] = centerX + (int) (r * cos((fovEndAngle-5)* 2 * M_PI / 360.0));
	fovRayEnd2[3] = centerY + (int) (r * sin((fovEndAngle-5)* 2 * M_PI / 360.0));
	...
	_min = _max = min(Distance(pointStart, centerPlane),Distance(pointEnd, centerPlane))- (1.01 + 1215 - 1106+80) ;
	///if (m_FovAngle > 150) _min = _max = _min = _max = min(Distance(pointStart, centerPlane),Distance(pointEnd, centerPlane))- (1.01 + 1215 - 1106)-4002;
//_min = _max = min(Distance(pointStart, centerPlane),Distance(pointEnd, centerPlane))- 1.01;
	...
	for (int i = 0; i < n; i++)
	{
 	 	int wall[4];
		m_Map->GetLine(wall, i);
		m_Map->SetVisibleLine(i, false);

		/*if (i != 30 && i != 41 && i!=56)
		{
		   	  		   continue;
  		}*/
		if (i<93 || i>114 || i==110||i==111|| i==112 || i==114)
			//continue;
			;
		if (i!=172 && i!=111 && i!=90 && i!=61 && i!=46 && i!=36)
		{
		...
		/*if (i!=111 && i!=112 && i!=128)
		{			
			continue;
		}*/

		if (i!=127 && i!=89 && i!=90 && i!=111 &&i!=74 &&i!=110 && i!=139 && i!=140 && i!=128 && i!=126)
		{
			//continue;
		}

		if (/*i!=110 /*&& i!=127*/ /*&& */i!=139 &&i!=169)
		{
			//continue;
		}
...
int Counter = 0;
	bool changed = true;
int _time_ = GetTickCount();

static vector<int> added(4096);
static vector<int> removed(4096);
static vector<int> distance0(4096);
/*added.clear();
removed.clear();
distance0.clear();*/
added.resize(0);
...
{	
	...
	for (int i = 0; i < nNodes; i ++ )
	{
		...
		if (Distance(cur->from, cur->to)< 0.01)
		{
			...
			//if (0 == count(distance0.begin(), distance0.end(), cur->lineIdx))
			{
				distance0.push_back(cur->lineIdx);
			}
		}
		...
		{
		 	Node * next = &nodes[j];
			bool On = IsOn(cur->min,  next);
  	 		On = On;


			static vector<int> lines(4096);
			lines.resize(0);
			for (int z = 0; z < nNodes; z ++)
			{
				//lines.push_back(nodes[z].lineIdx);
			}


		  	if (cur != next && !IsEqual(next->min, next->max) )
  	 		{
				Counter ++;
				// si "cur" plus près et qu il cache "next"	
		   		//if (max(cur->distFrom, cur->distTo) < min(next->distFrom, next->distTo)
				...
				/*if (IsEqual(cur->min, cur->max))
				{
					...
					cur->distFrom = last->distFrom;
					cur->distTo = last->distTo;
			   		nNodes --;
			   		changed = true;
					continue;
				}*/
		   		...
		   		if (IsOn(cur->min,  next))
		   		{
	 				...
	 				if (next->max <= cur->max)
	 				{
					   ...
					   {
							if (Distance(next->from, cur->from) < 0.001)
							{
								bool breakIt = true;
							}
					...
					else
					{
					   if (min (cur->distFrom, cur->distTo)< min(next->distFrom, next->distTo))
					   {
						...
						else
						 {
							...
							//if (0 == count(removed.begin(), removed.end(),cur->lineIdx))
							{
								//removed.push_back(Counter);
								//removed.push_back(cur->lineIdx);
							}
						  	Node  * last= &nodes[nNodes - 1];
						  	cur->min = last->min;
			   		   	   	cur->max = last->max;
...
/*
		...
		/*unique(added.begin(), added.end(),IsEqual);
		unique(removed.begin(), removed.end(),IsEqual);
		unique(distance0.begin(), distance0.end(),IsEqual);*/
	...
	nb = nNodes;
	nNodes =  SortNodesByDistance(nodes, nNodes);


_time_ = GetTickCount() - _time_;
	...
	for (int i = 0; i < nNodes; i ++)
	{
		m_Map->SetVisibleLine(nodes[i].lineIdx, true);
	}
	
	static int respoints[1024];
	int nbrespoints = 0;
...
//	 fprintf(ff,"%d lineIdx = %d min=%f max=%f  distForm=%f distTo=%f\n", i,idx, nodes[i].min,  nodes[i].max, nodes[i].distFrom, nodes[i].distTo);	
	 ...
	 GetIntersectionBetween2Lines(p0, ray0, wall);
 	 ...
 	 g->SetColor(0xff0000);
 	 g->FillRect((int) (0.5 + p0[0] - 0), (int) (0.5 + p0[1] - 0), 1, 1);
  	 g->FillRect((int) (0.5 + p1[0] - 0), (int) (0.5 + p1[1] - 0), 1, 1);
...
void Actor::UpdateDistanceNode(Node * n)
{
   double ray[4];
   double line[4];
   double p[2];
   ...
   if (n->distFrom * n->distFrom< 999999999.99 && !IsEqual(n->min, n->max))
   {
   //if (true)  return;

	ray[0] = (double) m_X + CELL_W / 2;
	ray[1] = (double) m_Y + CELL_H / 2;
	ray[2] = n->from[0];
	ray[3] = n->from[1];
	m_Map->GetLine(line, n->lineIdx);
	GetIntersectionBetween2Lines(p,ray, line);
	n->distFrom = Distance(ray, p);

	ray[2] = n->to[0];
	ray[3] = n->to[1];
	GetIntersectionBetween2Lines(p,ray, line);
	n->distTo = Distance(ray, p);
   }
