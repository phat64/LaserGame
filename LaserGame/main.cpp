...
Actor * actors[3] =
{
 	...
 	//new Actor(512, 320, 0, 0xff0000),
	//new Actor(384, 160, 0, 0xff0000),
	//new Actor(320, 256, 0, 0xff0000),
	//new Actor(352, 288, 0, 0xff0000),
	new Actor(384, 128, 0, 0xff0000),
                    ...
                    int iCmdShow)
{
    ...
    wc.lpszClassName = L"OGLSample";
    RegisterClass (&wc);

    /* create main window */
    hWnd = CreateWindow (
      L"OGLSample", L"Laser Game", 
	...
	actors[0]->SetDirection(DIR_LEFT);
	actors[1]->SetDirection(DIR_DOWN);
	actors[2]->SetDirection(DIR_LEFT);
...
{			 
		...
		/*	FILE * f = fopen("position.txt","w+");
			...
			g->SetColor(0x000000);
			hud->RenderNumber(g, AREA_X, AREA_Y + AREA_H + 50, lineIdx);
			hud->RenderNumber(g, AREA_X, AREA_Y + AREA_H + 50 +2*FONT_H, map->IsVisibleLine(lineIdx));
			...
			time = GetTickCount() - time;           // Sleep (1000/60);
            time = time;           

			int delay = 1000 /30 - time;
			if (delay > 0)
			Sleep (delay);
